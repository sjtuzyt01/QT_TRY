...
void MainWindow::updatePosition(int *x,int *y){
    for(int i=0;i<23;i++)
    {
        QLabel *labelChange=PlayerToLabel(i);
        if(TrackingStatus[i]==NOT_INCLUDED)
        {
            labelChange->setText("NOT TRACKING");
        }
        if(TrackingStatus[i]==OUT_OF_WINDOW)
        {
            labelChange->setText("OUT OF WINDOW");
        }
        if(TrackingStatus[i]==REGULAR)
        {
            labelChange->setText("x:"+QString::number(x[i],10)+",y:"+QString::number(y[i],10));
        }
    }
